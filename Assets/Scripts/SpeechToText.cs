...
public class SpeechToText : MonoBehaviour
{
    ...
    // We define four different status for SpeechToText component.
    public enum ProcessingStatus { Idle, Listening, Thinking, Talking };
    ...
    {
        status = ProcessingStatus.Listening;
        Runnable.Run(CreateService());
    }
    // A method for getting SpeechToText status.
    public ProcessingStatus GetStatus()
    {
        return status;
    }
    // A method for setting SpeechToText status.
    ...
    {
        // Avatar starts thinking what it should be the answer
        ...
        if (result != null && result.results.Length > 0)
        {
            foreach (var res in result.results)
            {
                foreach (var alt in res.alternatives)
                {
                    ...
                    if (res.final)
                    {
                        // Processing is end now it will start reading.
                        status = ProcessingStatus.Talking;
                    }
                }
